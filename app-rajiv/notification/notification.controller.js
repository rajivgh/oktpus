...
.controller('NotificationController', ['$log', '$scope', '$state','$cookies', '$localStorage', '$window', '$filter', '$timeout', '$sessionStorage', 'oktpusServiceFactory', 
	function ($log, $scope, $state, $cookies, $localStorage, $window, $filter, $timeout, $sessionStorage, oktpusServiceFactory) {
		...
		$scope.onEnter = function() {
			$scope.$storage.currentState = 'notification';
		...
		$scope.formatDateDisplay = function(raw_value) {
			...
			var formated_value = "";
			var tempValue = new Date(raw_value);
			formated_value += tempValue.getDate() + "-" + tempValue.getMonth() + "-" + tempValue.getFullYear() + " " + tempValue.getHours() + ":" + tempValue.getMinutes();
		...
		$scope.loadNotificationList = function() {
			...
			oktpusServiceFactory.getWithoutQueryString(url, function(response) {
                if(undefined != response.status && 1 == response.status) {
					...
					$scope.userHasProductFlag =  response.user_has_product_flag;				
				  } else if(0 == response.status && 401 == response.code) {
					  $scope.busy = false;
					  $log.debug("GetNotificationList:error");
					  $log.debug(response);
					  $scope.onExit("login");
					  $state.go('login');
                }
