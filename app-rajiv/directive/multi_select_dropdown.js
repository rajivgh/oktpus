...
angular.module( 'multiSelectDropDown', ['ng'])
.directive( 'multiSelectDropDown' , [ '$sce', '$timeout', '$templateCache', 'orderByFilter', '$filter',
function ( $sce, $timeout, $templateCache, orderBy, $filter) {
	return {
        restrict: 
            'AE',
        scope: {   
         ...
         */
         ...
         templateUrl: 
            'multi_select_dropdown.html',                            

         link: function ( $scope, element, attrs ) {                       

			$scope.backUp           = [];
            $scope.varButtonLabel   = $sce.trustAsHtml( $scope.translation.nothingSelected + '<span class="caret"></span>' );               
            ...
            $scope.$watch('closeDropDown', function (value) {
                // checking if the given value is not undefined
                if(value){
					$scope.Obj = value;
					// injecting the Method
                    $scope.Obj.invoke = function(){
						...
						//$scope.varButtonLabel = $sce.trustAsHtml( '<span class="dropDownLabel">' + $scope.lang.nothingSelected + '</span>');
						$scope.varButtonLabel = $sce.trustAsHtml( '<span class="dropDownLabel">' + $scope.lang.nothingSelected + '</span>' + '<span class="caret"></span>' + '<div class="dropDownSelected">' + $scope.lang.defaultDisplay +'</div>');
            ...
            $scope.$watch('openDropDown', function (value) {
                // checking if the given value is not undefined
                if(value){
					$scope.Obj = value;
					// injecting the Method
            ...
            $scope.$watch('changeDisplayLabel', function (value) {			
				// checking if the given value is not undefined
                if(value){
					$scope.Obj = value;
					// injecting the Method
                    $scope.Obj.invoke = function(label){
						$scope.lang.defaultDisplay = label;
                    }
                }  
			});
            
            $scope.$watch('refreshButtonText', function (value) {
				// checking if the given value is not undefined
                if(value){
					$scope.Obj = value;
					// injecting the Method
            ...
            $scope.refreshButtonTemp = function(value) {
			...
			/*		$scope.varButtonLabel   = $sce.trustAsHtml( $scope.translation.nothingSelected + '<span class="caret"></span>' );                
					...
					if ( $scope.outputModel.length === 0 ) {
                        ...
                        if(attrs.secondaryLableForCaller) {
                            $scope.secondaryLableForCaller = "all cities";
                        }
                    }
					else if($scope.outputModel.length == 1) {
						$scope.varButtonLabel = $sce.trustAsHtml( '<span class="dropDownLabel">' + $scope.translation.nothingSelected + '</span>' + '<span class="caret"></span>' + '<div class="dropDownSelected">' + $scope.outputModel[0].value +'</div>');
						//$scope.varButtonLabel += $scope.outputModel[0].value;
					    
                        if(attrs.secondaryLableForCaller) {
                            $scope.secondaryLableForCaller = $scope.outputModel[0].value;
                        }
                    }
					else if($scope.outputModel.length > 1) {
						$scope.varButtonLabel = $sce.trustAsHtml( '<span class="dropDownLabel">' + $scope.translation.nothingSelected + '</span>' + '<span class="caret"></span>' + '<div class="dropDownSelected">' + $scope.outputModel.length + ' selected' + '</div>');
						//$scope.varButtonLabel += $scope.outputModel.length + "Selected";
					    
                        if(attrs.secondaryLableForCaller) {
                            $scope.secondaryLableForCaller = $scope.outputModel.length + ' selected citie(s)';
                        }
                    }      */  
				}
            }
            ...
            $scope.refreshButton = function() {            
              /*  if(false == isRefreshButtonLabel && false == $scope.isDesktop) {
                    return;
				}
			*/
                $scope.varButtonLabel   = $sce.trustAsHtml( $scope.translation.nothingSelected + '<span class="caret"></span>' );                
                ...
                if ( $scope.outputModel.length === 0 ) {	
					...
					$scope.varButtonLabel = $sce.trustAsHtml( '<span class="dropDownLabel">' + $scope.translation.nothingSelected + '</span>' + '<span class="caret"></span>' + '<div class="dropDownSelected">' + ' All selected(' + $scope.outputModel.length +')'+'</div>');
				}
                else if($scope.outputModel.length <= 2) {
					$scope.varButtonLabel = '<span class="dropDownLabel">' + $scope.translation.nothingSelected + '</span>' + '<span class="caret"></span>' + '<div class="dropDownSelected">'  ;
					var i = 0;
					var geoLabel = '';
					angular.forEach($scope.outputModel, function( value, key ) {
						i++;
						var cityName = ($scope.outputModel[key].value).split(",");
						$scope.varButtonLabel += cityName[0] ; 
						if(attrs.secondaryLableForCaller) {							
								geoLabel +=  cityName[0];												
						}
						if(i != $scope.outputModel.length){
							$scope.varButtonLabel += ', ';
							geoLabel += ', ';
						}
                    }); 
                    
                    $scope.varButtonLabel +=  '</div>';
					$scope.varButtonLabel =  $sce.trustAsHtml($scope.varButtonLabel);	
					if(attrs.secondaryLableForCaller) {
                            $scope.secondaryLableForCaller = geoLabel;
                    }			                        
                }                
                else if($scope.outputModel.length > 2) {
					$scope.varButtonLabel = $sce.trustAsHtml( '<span class="dropDownLabel">' + $scope.translation.nothingSelected + '</span>' + '<span class="caret"></span>' + '<div class="dropDownSelected">' + $scope.outputModel.length + ' selected' + '</div>');
            ...
            $scope.writeLabel = function( item, type ) {
                ...
                angular.forEach( temp, function( value, key ) {
                    ...
                    if(value.toUpperCase() === 'COUNT' && item[value] != undefined) {
						var temp = '('+item[ value ]+')';
						//~ console.log(temp);
						//~ console.log(label);
					}
            ...
            $scope.toggleCheckboxes = function( e ) { 
				if(isPopupOpened == false)  {   
					isPopupOpened =true;                                
					/*$scope.searchformScroller = {
						'height' : '100vh !imprtant',
						'overflow-y' : 'hidden'
					}*/
					// We grab the button
					var clickedEl = element.children()[0];
					...
					// close
					if ( angular.element( checkBoxLayer ).hasClass( 'show' )) {   
						$scope.$parent.bottomWrapperPosition = "";
						$scope.$parent.formFieldMargin = "form-field-moremargin";                      

						if(false == $scope.isDesktop) {
							$scope.inputModel = angular.copy($scope.tempInputModel);
						}
						...
						$scope.tempInputModel = angular.copy($scope.inputModel);
					...
					// open
						...
						$scope.temp2;
						$scope.temp3;   
						$scope.isDDVisible = true;
						$scope.$parent.bottomWrapperPosition = "bottom-wrapper-position";
						$scope.$parent.formFieldMargin = "form-field-margin";     					
						$scope.inputModel = $filter('orderBy')($scope.inputModel, ['ticked', '-value'], true); 

						if(false == $scope.isDesktop) {
							$scope.tempInputModel = angular.copy($scope.inputModel);
							$scope.$parent.isPopUpOpen = true;
							$scope.$parent.tempClassForPopUpOpen = "temp-class-for-popup-open";       
							$scope.$parent.searchScroller = "search-scroller";        

								if($scope.inputModel.length > 5) {
									$scope.tempCheckboxContainer = {
									 //   'height' : '53vh'
									}
							...
							if($scope.inputModel.length > 5) {
								...
								$scope.temp1 = '' + (36 + (35*$scope.inputModel.length)) + 'px';
								$scope.temp2 = '' + (35*$scope.inputModel.length) + 'px';
								$scope.temp3 = "72px";
							}

							$scope.tempCheckboxLayer = {
								'height' : $scope.temp1,
								'top' : $scope.temp3
							}
						...
						if ( element[ 0 ].querySelector( '.inputFilter' ) ) {                        
							 element[ 0 ].querySelector( '.inputFilter' ).focus();    
							 element[ 0 ].querySelector( '.inputFilter' ).blur();
							$scope.tabIndex = $scope.tabIndex + helperItemsLength - 2;
							// blur button in vain							
							angular.element( element ).children()[ 0 ].blur();
						}
						...
						//e.preventDefault();
						//var target= $('#'+$scope.directiveId);
						/*$('html, body').stop().animate({
							scrollTop: $('#'+$scope.directiveId).offset().top
						}, 1000);*/
            ...
            $scope.externalClickListener = function( e ) {   
                ...
                if(false == $scope.isDesktop && false == $scope.isDDVisible) {
                    $scope.inputModel = angular.copy($scope.tempInputModel);
                }
            ...
            $scope.popupName        = attrs.popupName;
            ...
            if ( typeof attrs.translation !== 'undefined' ) {
                ...
                $scope.lang.defaultDisplay = $sce.trustAsHtml( $scope.translation.defaultDisplay );                 
            }
            else {
                ...
                $scope.lang.defaultDisplay = 'Select';                          
            }
            ...
            $scope.searchBoxFocusChange = function(value) {
				if(true == value) {
					$scope.isDDVisible = true;
				}
				else {
    ...
    var template = 
        '<span class="multiSelect inlineBlock">' +
            ...
            '</button>' +
            // overlay layer
            //'<div class="checkboxLayer" ng-class="{dropDownFixedHeight: filteredModel.length > 4, dropDownVariableHeight: filteredModel.length < 5}">' +
            '<div class="checkboxLayer" ng-style="{{tempCheckboxLayer}}" ng-class="{temp1ClassFoorHeight: inputModel.length > 4, temp2ClassFoorHeight: inputModel.length < 4, temp3ClassFoorHeight: inputModel.length == 4 }">' + 
                '<div class="filter-name-wrapper" ng-click="toggleCheckboxes($event)">' +
                  '<span class="filter-name"> {{popupName}} </span>' +
                ...
                '<div class="helperContainer" ng-if="helperStatus.filter || helperStatus.all || helperStatus.none || helperStatus.reset ">' +
                    ...
                    '<div class="line" ng-if="helperStatus.all || helperStatus.none || helperStatus.reset ">' +
                        // select all
                        '<button type="button" id="selectAll" class="helperButton"' +
                        ...
                        '</button>'+
                        // select none
                        '<button type="button" id="selectNone" class="helperButton"' +
                            'ng-disabled="isDisabled"' + 
                        ...
                        '</button>'+
                        // reset
                        '<button type="button" id="reset" class="helperButton reset"' +
                            'ng-disabled="isDisabled"' + 
                    ...
                    '<div class="line" style="position:relative" ng-if="helperStatus.filter">'+
                        ...
                        '<input placeholder="{{lang.search}}" type="text"' +
                            ...
                            'ng-change="searchChanged()" class="inputFilter"'+
                             'ng-focus="searchBoxFocusChange(true)"'+
                            'ng-blur="searchBoxFocusChange(false)"/>'+
                        // clear button
                        '<img src="assets/images/search_remove.png" class="clearButton" ng-click="clearClicked( $event )" > '+
                    '</div> '+
                '</div> '+
                // selection items
                '<div class="checkBoxContainer" ng-style="tempCheckboxContainer">'+
                    '<div '+
                    ...
                    '<div class="acol">'+
                        '<label>'+                                
                            ...
                            //~ '<span '+
                                ...
                                //~ 'ng-bind-html="writeLabel( item, \'itemLabel1\' )">'+
                            //~ '</span>'+
                            '<span '+
                                'ng-class="{disabled:itemIsDisabled( item )}" >'+
                                '{{item.value}}'+
                            '</span>'+
                            '<span ng-show="true == isCountRequired"'+
                                'ng-class="{disabled:itemIsDisabled( item )}" >'+
                                ' ({{item.count}})'+
                            '</span>'+
                        '</label>'+
                ...
                '</div>'+                
            '</div>'+
            '<div class="search-btn" ng-click="okClicked()">OK</div>'+
        '</div>'+
    '</span>';
	$templateCache.put( 'multi_select_dropdown.html' , template );
}]); 
